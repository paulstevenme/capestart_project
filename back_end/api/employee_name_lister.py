...
employee_name_lister_code = Blueprint('employee_name_lister_code', __name__)


@employee_name_lister_code.route('/listEmployeeNames', methods=['POST'])
def list_company_name():
    try:
        ...
        total_db_datas = Employees.query.filter_by(company_id=compid).count()
        companyDetailData = Employees.query.filter_by(company_id=compid).all()
        for i in companyDetailData:
            dd = {'id': i.id, 'name': i.employee_name, 'data': i.employee_salary}
            db_data.append(dd)
